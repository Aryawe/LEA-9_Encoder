...
for %%f in (unlocked/*.*) do (
  FileEncoder3.exe %key1% %key2% %key3% %key4% %key5% %key6% %key7% %key8% %key9% "unlocked/%%f" "%temp%/%%f.tmp"
)

echo Second pass...

for %%f in (unlocked/*.*) do (
  FileEncoder3.exe %key1% %key2% %key3% %key4% %key5% %key6% %key7% %key8% %key9% "%temp%/%%f.tmp" "locked/%%f"
  del /Q /F "%temp%\%%f.tmp"
  del /Q /F "unlocked\%%f"
)
